...
namespace HeadacheCDSSWeb.Controllers
{
    public class ViewPatRecordController : Controller
    {
        ...
        public ActionResult Index(string ID)
        {
            ...
            List<mVisitRecord> Lvisit = visitop.GetVistRecord(ID);
            if (Lvisit!=null&&Lvisit.Count != 0)
            {
                this.TempData["recordID"] = Lvisit.First().VisitrecordId.ToString();
                this.ViewBag.recordId = Lvisit.First().VisitrecordId.ToString();
            }
            return View(Lvisit);
        }
